                ...
                dirtab_bars[]=	 {{".",		QTDIR,		FsDBars,	0700|DMDIR },
                    ...
                    dirtab_tags[]=	 {{".",		QTDIR,		FsDTags,	0500|DMDIR },
                        ...
                        dirtab_tag[]=	 {{".",		QTDIR,		FsDTag,		0500|DMDIR },
                            ...
                            {nil}};
static std::map<uint, IxpDirtab*> dirtab {
    {FsRoot, dirtab_root},
        {FsDBars, dirtab_bars},
...
std::map<uint, ActionTab> actiontab {
    {FsFBar, ActionTab { .msg = (MsgFunc)message_bar,          .read = (BufFunc)readctl_bar }},
        {FsFCctl,      ActionTab { .msg = (MsgFunc)message_client,     	.read = (BufFunc)readctl_client }},
        {FsFRctl,      ActionTab { .msg = (MsgFunc)message_root,       	.read = (BufFunc)readctl_root }},
        {FsFTctl,      ActionTab { .msg = (MsgFunc)message_view,       	.read = (BufFunc)readctl_view }},
        {FsFTindex,    ActionTab { .msg = (MsgFunc)0,		    	.read = (BufFunc)view_index }},
        {FsFColRules,  ActionTab { .buffer = offsetof(Ruleset, string),	.size = offsetof(Ruleset, size) }},
        {FsFKeys,      ActionTab { .buffer = offsetof(Defs, keys),	.size = offsetof(Defs, keyssz) }},
        {FsFRules,     ActionTab { .buffer = offsetof(Ruleset, string), 	.size = offsetof(Ruleset, size) }},
        {FsFClabel,    ActionTab { .buffer = offsetof(Client, name),	.max = sizeof ((Client*)0)->name }},
        {FsFCtags,     ActionTab { .buffer = offsetof(Client, tags),   	.max = sizeof ((Client*)0)->tags }},
        {FsFprops,     ActionTab { .buffer = offsetof(Client, props),  	.max = sizeof ((Client*)0)->props }},
};
...
dwrite(int flag, void *buf, int n, bool always) {
    ...
    if(debugfile&flag)
        for(i=0; i < nelem(pdebug); i++)
            if(flag & (1<<i)) {
                ixp_pending_write(pdebug+i, (const char*) buf, n);
            }
}
...
static uint
fs_size(IxpFileId *f) {
    if (auto t = actiontab.find(f->tab.type); t != actiontab.end()) {
        if(t->second.size) {
            return structmember(f->p.ref, int, t->second.size);
        } else if(t->second.buffer && t->second.max) {
            return strlen(structptr(f->p.ref, char, t->second.buffer));
        } else if(t->second.buffer) {
            return strlen(structmember(f->p.ref, char*, t->second.buffer));
        } else if(t->second.read) {
            return strlen(t->second.read(f->p.ref));
        }
    }
    return 0;
}
...
fs_read(Ixp9Req *r) {
    ...
    if(f->tab.perm & DMDIR && f->tab.perm & 0400) {
        ...
        if (auto t = actiontab.find(f->tab.type); t != actiontab.end()) {
            if(t->second.read)
                buf = t->second.read(f->p.ref);
            else if(t->second.buffer && t->second.max)
                buf = structptr(f->p.ref, char, t->second.buffer);
            else if(t->second.buffer)
                buf = structmember(f->p.ref, char*, t->second.buffer);
            else
                goto done;
            n = t->second.size ? structmember(f->p.ref, int, t->second.size) : strlen(buf);
...
fs_write(Ixp9Req *r) {
    ...
    if (auto t = actiontab.find(f->tab.type); t != actiontab.end()) {
        if(t->second.msg) {
            errstr = ixp_srv_writectl(r, t->second.msg);
            r->ofcall.io.count = r->ifcall.io.count;
        } else if(t->second.buffer && t->second.max) {
            char* temporary[] = { structptr(f->p.ref, char, t->second.buffer) };
            uint temporary1[] = { strlen(structptr(f->p.ref, char, t->second.buffer)) };
            ixp_srv_writebuf(r, temporary,
                    t->second.size ? structptr(f->p.ref, uint, t->second.size) : temporary1,
                    t->second.max);
        } else if(t->second.buffer) {
            ixp_srv_writebuf(r, structptr(f->p.ref, char*, t->second.buffer),
                    t->second.size ? structptr(f->p.ref, uint, t->second.size) : nil,
                    t->second.max);
        } else {
            goto done;
        }
...
fs_create(Ixp9Req *r) {
    ...
    switch(f->tab.type) {
        ...
        case FsDBars:
            ...
            f->next = (decltype(f->next))r->fid->aux;
            r->fid->aux = f;
            ixp_respond(r, nil);
            break;
    }
